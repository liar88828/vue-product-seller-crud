...
import { CheckOutDB } from "../transaction/order/CheckOutDB"

export type IdTrollyProduct = {
  id_trolly: number
  id_product: number
  id_user: string
}
...
export class TrollyDB extends TrollyMutation {
  ...
  async all({ id_trolley, id_user }: IdTrolly): Promise<TollyProps[]> {
    return prisma.trolly.findMany({
      where: { id: id_trolley, User: { id: id_user } },
      include: {
        Box: {
          include: {
            Product: true,
  ...
  async productId({
    ...
    id_product,
    id_user,
  }: IdTrollyProduct): Promise<BoxProduct[]> {
    const data = await prisma.box.findMany({
      where: {
        id_trolly,
        id_product,
